...
function zoo_button_wishlist() {
	if ( get_theme_mod( 'zoo_enable_wishlist', '1' ) == 1 ) {
		...
		echo apply_filters( 'zoo_button_wishlist', sprintf(
			...
			get_theme_mod( 'zoo_text_add_to_wishlist', esc_attr__( 'Add to wishlist', 'ciao' ) ),
			esc_attr__( 'Product Added', 'ciao' ),
			$icon,
			get_theme_mod( 'zoo_text_add_to_wishlist', esc_html__( 'Add to wishlist', 'ciao' ) )
		) );
	} else {
		return false;
	}
...
function zoo_button_products_compare() {
	if ( get_theme_mod( 'zoo_enable_compare', '1' ) == 1 ) {
		...
		echo apply_filters( 'zoo_button_products_compare', sprintf(
			...
			$class,
			esc_attr( $product->get_id() ),
			get_theme_mod( 'zoo_text_add_to_compare', esc_attr__( 'Add to compare', 'ciao' ) ),
			esc_attr__( 'Product Added', 'ciao' ),
			$icon,
			get_theme_mod( 'zoo_text_add_to_compare', esc_html__( 'Add to compare', 'ciao' ) )
		) );
	} else {
		return false;
	}
...
if ( ! function_exists( 'zoo_template_quick_view_btn' ) ) {
	function zoo_template_quick_view_btn() {
		$quick_view_btn = '<a href="#" class="button btn-quick-view" title="' . esc_attr__( 'Quick view', 'ciao' ) . '" data-productID="' . get_the_ID() . '"><i class="zoo-icon-eye"></i> ' . esc_html__( 'Quick view', 'ciao' ) . '</a>';
...
if ( ! function_exists( 'zoo_shop_stock_label' ) ) {
	function zoo_shop_stock_label() {
		...
		if ( ! $product->is_in_stock() ) {
			?>
            <span class="out-stock-label zoo-stock-label"><?php esc_html_e( 'Out of Stock', 'ciao' ); ?></span>
			<?php
		} else {
			if ( get_option( 'woocommerce_notify_low_stock_amount' ) > $product->get_stock_quantity() && $product->get_stock_quantity() ) {
				?>
                <span class="low-stock-label zoo-stock-label"><?php esc_html_e( 'Low Stock', 'ciao' ); ?></span>
				<?php
			}
		}
	}
...
function zoo_close_wrap_no_product() {
	...
	echo wp_kses( '<a href="' . get_permalink( wc_get_page_id( 'shop' ) ) . '" title="' . esc_attr__( 'Return to Shop', 'ciao' ) . '" class="button return-shop">' . esc_html__( 'Return to Shop', 'ciao' ) . '</a>', $allow_html );
	echo wp_kses( '</div>', $allow_html );
}
