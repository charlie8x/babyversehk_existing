...
if(!function_exists('zoo_custom_gallery_img_sizes')){
    function zoo_custom_gallery_img_sizes($imgsizes)
    {
        return array_merge($imgsizes, array(
            'zoo_gallery_thumb' => esc_html__('Custom Product Gallery Thumbnail', 'ciao'),
        ));
    }
}
 ...
 */
add_action( 'woocommerce_before_main_content', 'zoo_single_product_top_nav',21 );
...
if ( ! function_exists( 'zoo_before_single_product' ) ) {
	function zoo_before_single_product() {
		...
		switch ( zoo_single_product_content_layout() ) {
			case 'left_content':
				$classes .= ' container left-content-layout content-half-width';
				break;
			case 'sticky_content':
				wp_enqueue_script( 'sticky-kit' );
				$classes .= ' container sticky-content-layout content-half-width';
				break;
			case 'center':
			...
			case 'full_content':
				...
				break;
			default:
				$classes .= ' container right-content-layout content-half-width';
				break;
		};
...
if ( ! function_exists( 'zoo_product_video' ) ) {
	function zoo_product_video() {
		...
		if ( $zoo_product_video != '' ) {
			...
			$zoo_pv_html = '<div class="wrap-product-video ' . $zoo_embed_class . '"><a href="' . $zoo_product_video . '"  class="button video-lb-control" title="' . esc_attr__( "Watch Video", 'ciao' ) . '"><i class="cs-font clever-icon-play-1"></i><span>' . esc_html__( "Watch Video", 'ciao' ) . '</span></a>';
...
if(!function_exists('zoo_sale_count_down_variable_html')){
	function zoo_sale_count_down_variable_html($variation){
		...
		if($sale_date > time()) {
			...
			$html .= '<span class="label-product-countdown">'.esc_html__("Hurry Up! Sales end in","zootemplate").'</span>';
			...
			if(get_theme_mod('zoo_product_stock_countdown',1)==1){
				...
				if($zoo_base_stock!=0 && $stock!=NULL) {
	                ...
	                $html .= esc_html__( 'Hurry! Only ', 'ciao').'<b class="number-product">'.$stock.'</b>'.esc_html__(' left in Stock.', 'ciao' );
	                $html .= '</span>';
...
function zoo_single_product_new_label() {
	...
	if ( get_post_meta( $product->get_id(), 'zoo_single_product_new', true ) == 1 ) {
		?>
        <span class="zoo-new-label"><?php esc_html_e( 'New', 'ciao' ); ?></span>
		<?php
	}
}
...
function zoo_wc_get_stock_html() {
	...
	$label= $availability['class']!='out-of-stock'?esc_html__('In stock','ciao'):$availability['availability'];
...
add_action('zoo_custom_before_product_meta','woocommerce_template_single_rating',0);

/**
 * Zoo single product sku.
 * Move product sku to before price.
 * @uses hook to woocommerce_template_single_price
 * @return html sku
 */
if ( ! function_exists( 'zoo_single_product_sku' ) ) {
	function zoo_single_product_sku() {
		get_template_part( 'inc/templates/woocommerce/single-product/product-custom', 'meta' );
	}
}
add_action( 'woocommerce_single_product_summary', 'zoo_single_product_sku', 10 );

/**
 *  Change order hook woocommerce_template_single_price for display after product SKU.
 */
...
if ( ! function_exists( 'zoo_recently_viewed_product' ) && get_theme_mod( 'zoo_enable_recently_viewed_product', 1 ) == 1 ) {
	function zoo_recently_viewed_product() {
		...
		if ( $the_query->have_posts() ) {
			...
			esc_html_e( 'Recently viewed products', 'ciao' );
			echo wp_kses( '</h2>', $allow_html );
