				...
				if ( is_array( $min_amounts ) && $min_amounts ) {
					...
					foreach ( WC()->cart->get_coupons() as $code => $coupon ) :
						if(!$coupon->enable_free_shipping()) {
							if($coupon->get_discount_type()=="percent"){
								$current -= ($coupon->get_amount())*$current/100;
							}else{
					...
					if ( $current > 0 ) {
						...
						if ( $current < $min_amount ) {
							$added_text .= esc_html__( 'Spend ', 'ciao' ) . wc_price( $min_amount - $current ) . esc_html__( '  to get Free Shipping ', 'ciao' );
						} else {
							$added_text .= esc_html__( 'Congratulations! You\'ve got free shipping!', 'ciao' );
						}
...
function zoo_login_button() {
	...
	echo wp_kses( '<p class="form-row  wrap-login-toggle-button"><a href=# class="toggle-login-form button toggle-button" title="' . esc_attr__( 'Login', 'ciao' ) . '">' . esc_html__( 'Login', 'ciao' ) . '</a></p>', $allow_html );
}
...
function zoo_register_button() {
	...
	if ( get_option( 'woocommerce_enable_myaccount_registration' ) === 'yes' && ! is_checkout() ) {
		echo wp_kses( '<p class="form-row wrap-register-toggle-button"><a href=# class="toggle-register-form button toggle-button" title="' . esc_attr__( 'Register', 'ciao' ) . '">' . esc_html__( 'Register', 'ciao' ) . '</a></p>', $allow_html );
	} else {
		return;
	}
}
...
if(!function_exists('zoo_woo_cart_trust_badges')){
    function zoo_woo_cart_trust_badges(){
        ...
        if(isset($trust_badges['url'])){
            echo sprintf('<img src="%s" class="trust-badges" alt="%s"/>',$trust_badges['url'], esc_attr__('Trust badges','ciao'));
        }else{
            return;
        }
    }
