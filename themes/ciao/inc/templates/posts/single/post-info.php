...
<ul class="post-info">
    ...
    if (get_theme_mod('zoo_blog_single_post_info_style', 'icon') == 'icon') {
        ...
        if (get_theme_mod('zoo_enable_blog_date_post', '1') == 1) {
            ?>
            <li class="post-date"><?php esc_html_e('on', 'ciao'); ?><?php echo esc_html(get_the_date()); ?></li>
        <?php }
        if (get_theme_mod('zoo_enable_blog_cat_post', '1') == 1) {
            ?>
            <li class="list-cat"><?php esc_html_e('in', 'ciao'); ?><?php echo get_the_term_list(get_the_ID(), 'category', '', ', ', ''); ?></li>
        <?php }
    }
    ?>
</ul>
