    ...
    <ul class="zoo-extend-notice">
		...
		if(get_theme_mod('zoo_product_get_order',1)==1){
			...
			if ( $availability['class'] != 'out-of-stock' ) {
                ...
                <?php
                echo esc_html__('Order in The Next', 'ciao');
                ?>
                        <span class="end-of-day" data-timezone="<?php echo get_option('gmt_offset');  ?>" data-hours="<?php esc_attr_e('hours', 'ciao') ?>"
                              data-minutes="<?php esc_attr_e('minutes', 'ciao') ?>">
                </span>
						<?php
						echo esc_html__(' to get it by', 'ciao');
						?>
		...
		if(get_theme_mod('zoo_product_get_visitor',1)==1 && class_exists('CleverOnePixel')){
            ...
            <li class="zoo-extend-notice-item zoo-count-visitor-notice">
                ...
                <span class="extend-notice-label">
                <?php
                esc_html_e('Real Time ','ciao');
                echo CleverOnePixel::countVisitors();
                esc_html_e(' Visitor Right Now','ciao');
                ?>
            </span>
            </li>
			<?php
			...
			if ( isset( $min_amounts ) ) {
				if ( is_array( $min_amounts ) && $min_amounts ) {
                    ...
                    if ($current < $min_amount) {
	                    echo esc_html__( 'Spend ', 'ciao' ) . '<span class="free-shipping-amount">' . wc_price( $min_amount - $current ) . '</span>' . esc_html__( ' to get Free Shipping', 'ciao' );
                    }else {
	                    esc_html_e('Congratulations! You\'ve got free shipping!', 'ciao');
                    }
                    ?>
                    </span>
                    </li>
