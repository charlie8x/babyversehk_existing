...
final class Zoo_Setup_Demo_Page
{
    ...
    function _add($context = '')
    {
        $this->hook_suffix = zoo_add_submenu_page(
            ...
            esc_html__('Setup Demo Data', 'ciao'),
            esc_html__('Setup Demo Data', 'ciao'),
            'manage_options',
            self::SLUG,
            array($this, '_render')
    ...
    function _render()
    {
        ...
        ?><div class="zoo-theme-setup-site-demo">
			<h2 class="screen-reader-text"><?php esc_html_e('Setup site demo', 'ciao') ?></h2>
                    ...
                    foreach ($demos as $folder) :
	                    ...
	                    ?><li class="demo-item <?php echo esc_attr($style === $demo ? 'current ' : ''); ?>">
                            <div class="demo-content">
                                ...
                                <?php if ($style === $demo) : ?>
                                    <span class="demo-label"><?php esc_html_e('Installed', 'ciao') ?></span>
                                <?php endif ?>
                            </div>
                            <div class="demo-footer">
                                <span class="demo-title"><?php echo esc_html($home_data['posts'][0]['post_title']) ?></span>
                                <span class="demo-actions">
                                    <img class="demo-spinner" src="<?php echo admin_url('images/spinner.gif') ?>" alt="<?php esc_attr_e('Installing', 'ciao') ?>">
                                    <a class="demo-preview" href="<?php echo esc_url($home_data['posts'][0]['_link']) ?>" target="_blank"><?php esc_html_e('Preview', 'ciao') ?></a>
                                    <span class="sep">|</span>
                                    <?php
                                        if ($style === $demo) {
                                            $button_name = esc_html__('Uninstall', 'ciao');
                                            $action = $button_class = 'uninstall';
                                        } else {
                                            $button_name = esc_html__('Install', 'ciao');
    ...
    function _setUpDemo()
    {
        if (!current_user_can('manage_options')) {
            exit(json_encode([
                'success' => false,
                'message' => esc_html__('Cheating, huh!', 'ciao')
            ]));
        }
        ...
        if (empty($_POST['zoo_demo_slug'])) {
            exit(json_encode([
                'success' => false,
                'message' => esc_html__('Invalid demo ID.', 'ciao')
            ]));
        }
        ...
        if (!is_readable($demo_path . $homef)) {
            exit(json_encode([
                'success' => false,
                'message' => esc_html__('Demo content file not found.', 'ciao')
            ]));
        }
		...
		if (isset($imported_content['posts'][$home_id])) {
            ...
            if (true !== $ok) {
                exit(json_encode([
                    'success' => false,
                    'message' => esc_html__('Failed to import demo content.', 'ciao') . ' ' . $ok->getMessage()
                ]));
            }
			update_option('show_on_front', 'page');
			update_option('page_on_front', $imported_content['posts'][$home_id]);
        ...
        exit(json_encode([
            'success' => true,
            'message' => esc_html__('Demo data installed successfully!', 'ciao')
        ]));
    }
    ...
    protected function installBaseData(Zoo_Demo_Importer $importer)
    {
        ...
        if (!$is_base_content_installed) {
            if (!is_readable(ZOO_THEME_DIR . 'inc/sample-data/base/content.xml')) {
                exit(json_encode([
                    'success' => false,
                    'message' => esc_html__('Base content file not found.', 'ciao')
                ]));
            }
            ...
            if ($ok !== true) {
                exit(json_encode([
                    'success' => false,
                    'message' => esc_html__('Failed to import base content.', 'ciao') . ' ' . $ok->getMessage()
                ]));
            }
    ...
    protected function installDemoData(Zoo_Demo_Importer $importer, $demo)
    {
        ...
        if ($ok !== true) {
            exit(json_encode([
                'success' => false,
                'message' => esc_html__('Failed to import site content.', 'ciao') . ' ' . $ok->getMessage()
            ]));
        }
    }
    ...
    function _uninstallDemo()
    {
        ...
        $this->removeOldDemo();
            ...
            exit(json_encode([
                'success' => true,
                'message' => esc_html__('Demo has been uninstalled successfully.', 'ciao')
            ]));
