...
final class Zoo_Settings_Page
{
    ...
    function _add($context = '')
    {
        ...
        $this->hook_suffix = zoo_add_submenu_page(
            ...
            esc_html__('Theme Settings', 'ciao'),
            esc_html__('Theme Settings', 'ciao'),
            'manage_options',
            self::SLUG,
            array($this, '_render')
    ...
    function _render()
    {
        ...
        <div class="zoo-theme-settings">
            <div class="nav-bar">
                ...
                <ul>
                    <li><a class="nav-item active-item" href="#global-settings"><?php esc_html_e('General', 'ciao') ?></a></li>
                    <li><a class="nav-item" href="#advanced-settings"><?php esc_html_e('Advanced', 'ciao') ?></a></li>
                    <li><a class="nav-item" href="#import-export"><?php esc_html_e('Import/Export', 'ciao') ?></a></li>
                </ul>
            </div>
            <div class="content-tabs">
                <form class="form-table" action="options.php" method="post">
                    ...
                    <table id="global-settings" class="tab-table active-tab">
                        <caption><?php esc_html_e('General Options', 'ciao') ?></caption>
                        <tr>
                            <?php $enable_mega_menu = intval($this->getValue('enable_builtin_mega_menu')) ?>
                            <th rowspan="scope"><?php esc_html_e('Enable Built-in Mega Menu Editor', 'ciao') ?></th>
                            <td>
                                <label>
                                    ...
                                    <span class="description"><?php esc_html_e('Whether to enable built-in mega menu or not.', 'ciao'); ?></span>
                                </label>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('Header scripts', 'ciao') ?></th>
                            <td><p><textarea name="<?php echo esc_attr($this->getName('header_scripts')) ?>" rows="6" cols="80"><?php echo wp_unslash($this->getValue('header_scripts')) ?></textarea></p><p class="description"><?php esc_html_e('Here come custom scripts inserted inside HEAD tag.', 'ciao') ?></p></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('Footer scripts', 'ciao') ?></th>
                            <td><p><textarea name="<?php echo esc_attr($this->getName('footer_scripts')) ?>" rows="6" cols="80"><?php echo wp_unslash($this->getValue('footer_scripts')) ?></textarea></p><p class="description"><?php esc_html_e('Here comes your Google Analytics code or any other JS code you want to be loaded in the footer of your website.', 'ciao') ?></p></td>
                        </tr>
                    ...
                    <table id="advanced-settings" class="tab-table">
                        ...
                        ?>
                        <caption><?php esc_html_e('Advanced Options', 'ciao') ?></caption>
                        <tr>
                            <th rowspan="scope"><?php esc_html_e('Enable Development Mode', 'ciao') ?></th>
                            <td>
                                <label>
                                    <input type="checkbox" name="<?php echo esc_attr($this->getName('enable_dev_mode')) ?>" value="1"<?php if ($enable_dev_mode) echo ' checked'; ?>>
                                    <span class="description"><?php esc_html_e('This option is for theme developers only. Use it with caution!', 'ciao'); ?></span></label>
                            </td>
                        </tr>
                        <tr>
                            <th rowspan="scope"><?php esc_html_e('Responsive Breakpoint for Mobile (px)', 'ciao') ?></th>
                            <td>
                                ...
                                <p class="description"><?php esc_html_e('Set a custom mobile breakpoint at which your site layout will be rendered for mobile devices. E.g. 768px. The default value is inherited from Bootstrap 4 - 992px.', 'ciao'); ?></p>
                            </td>
                        </tr>
                        <?php do_settings_fields(self::GROUP, 'advanced') ?>
                    </table>
                    <table id="import-export" class="tab-table">
                        <caption><?php esc_html_e('Import/Export Theme Options', 'ciao') ?></caption>
                        <tr>
                            <th scope="row"><?php esc_html_e('Import', 'ciao') ?></th>
                            <td>
                                <p class="description"><?php esc_html_e('Paste encoded settings saved from the "Export" box and click on the "Save Changes" to import.', 'ciao') ?></p>
                                <textarea name="<?php echo esc_attr($this->getName('import_settings')) ?>" rows="8" cols="80"></textarea>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('Export', 'ciao') ?></th>
                            <td>
                                <p class="description"><?php esc_html_e('Copy the below JSON encoded settings and save it somewhere to import later.', 'ciao') ?></p>
    ...
    function _notify()
    {
        ...
        if (isset($_REQUEST['settings-updated']) && 'true' === $_REQUEST['settings-updated']) {
            echo '<div class="updated notice is-dismissible"><p><strong>' . esc_html__('Settings have been saved successfully!', 'ciao') . '</strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'ciao') . '</span></div>';
        }

        if (isset($_REQUEST['error']) && 'true' === $_REQUEST['error']) {
            echo '<div class="updated error is-dismissible"><p><strong>' . esc_html__('Failed to save settings. Please try again!', 'ciao') . '</strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'ciao') . '</span></div>';
        }
    }
