...
final class Zoo_Setup_Wizard
{
	...
	{
		$this->theme = wp_get_theme(get_option('template', 'ciao'));
	...
	function _wizard_steps()
	{
		$steps = array(
			'introduction' => array(
				'name'    => esc_html__( 'Introduction', 'ciao' ),
				'view'    => array( $this, 'introduction' ),
				'handler' => array( $this, 'introduction_save' ),
			),
			'system_status' => array(
				'name'    => esc_html__( 'Preinstallation', 'ciao' ),
				'view'    => array( $this, 'system_status' ),
				'handler' => '',
			),
			'child_theme' => array(
				'name'    => esc_html__( 'Child Theme', 'ciao' ),
				'view'    => array( $this, 'child_theme' ),
				'handler' => '',
			),
			'default_plugins' => array(
				'name'    => esc_html__( 'Install Plugins', 'ciao' ),
				'view'    => array( $this, 'default_plugins' ),
				'handler' => '',
			),
			'done_setup' => array(
				'name'    => esc_html__( 'Done', 'ciao' ),
	...
	function _setup_wizard()
	{
		...
		wp_localize_script( 'envato-setup', 'envato_setup_params', array(
			...
			'verify_text'  => esc_html__( ' Verifying', 'ciao' ),
		) );
	...
	function _ajax_child_theme()
	{
		if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['cThemeName'] ) ) {
			wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Invalid child theme data. Please try again!', 'ciao' ) ) );
		}
 ...
 */\n";
		...
		if ( !file_exists($child_theme_path) ) {
			...
			wp_send_json( array( 'done' => 1, 'message' => sprintf( esc_html__('%s has already been created. Child theme files are stored in %s.', 'ciao' ), $child_theme_title, '<code>' . $child_theme_path . '</code>' ) ) );
		}
		...
		wp_send_json( array( 'done' => 1, 'message' => sprintf( esc_html__('%s has been created and activated successfully! Child theme files are stored in %s.', 'ciao' ), $child_theme_title, '<code><strong>' . $child_theme_path . '</strong></code>' ) ) );
	}
	...
	function _ajax_plugins()
	{
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'ciao' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'ciao' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'ciao' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			...
			wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'ciao' ) ) );
		}
	...
	function _ajax_content()
	{
		...
		if ( !check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[$_POST['content']] ) ) {
			wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Invalid content!', 'ciao' ) ) );
		}
	...
	protected function setup_wizard_header()
	{
		...
		<head>
			...
			<?php printf( esc_html__( '%s%sTheme &rsaquo; Setup Wizard%s', 'ciao' ), '<ti', 'tle>', '</title>' ) ?>
	...
	{
		?><a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'ciao' ); ?></a>
		</body><?php
	...
	protected function introduction()
	{
		if ( $this->is_possible_upgrade() ) :
			?><h1><?php esc_html_e( 'Welcome to theme setup wizard!', 'ciao' ); ?></h1>
			<p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'ciao' ); ?></p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button-primary button button-large"><?php esc_html_e( 'Let&#8217;s go!', 'ciao' ); ?></a>
				<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url() ); ?>"
				   class="button button-large"><?php esc_html_e( 'Not right now', 'ciao' ); ?></a>
			</p>
			<?php
		elseif ( get_option( $this->theme->template . '_theme_setup_completed' ) && true === $this->tgmpa->is_tgmpa_complete() ) :
			?>
			<h1><?php esc_html_e( 'Welcome to theme setup wizard!', 'ciao' ); ?></h1>
			<p class="lead success"><strong><?php esc_html_e( 'It looks like you have already run the setup wizard. If you want to run the wizard again, click on "Run setup wizard again".', 'ciao' ); ?></strong></p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Run setup wizard again', 'ciao' ); ?></a>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'ciao' ); ?></a>
			...
			?>
			<h1><?php esc_html_e( 'Welcome to theme setup wizard!', 'ciao' ); ?></h1>
			<p><?php printf(esc_html__( 'Thank you for choosing %s theme! This setup wizard is to help you setup your new website quickly. Step by step, you will install necessary plugins, import demo content, customize site appearance...', 'ciao' ), $this->theme->name); ?></p>
			<p><?php esc_html_e( 'If you don&#8217;t want to go through the wizard, you can skip any step or return to the WordPress admin dashboard. Come back anytime if you change your mind!', 'ciao' ); ?></p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Let&#8217;s go!', 'ciao' ); ?></a>
				<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url() ); ?>" class="button button-large"><?php esc_html_e( 'Not right now', 'ciao' ); ?></a>
			</p>
			<?php
		endif;
	}
	...
	{
		?><p><?php esc_html_e('Before installing the theme, we need to check your system environment. The theme will not work properly if your system does not meet the requirements.', 'ciao') ?></p>
        ...
        <tr>
            ...
            ?>
            <td class="field-label"><?php esc_html_e('XML Parser', 'ciao'); ?></td>
            <td>
                <span class="field-tooltip" href="#">
                    <span class="dashicons dashicons-info"></span>
                    <span class="tip-content">
                        <?php printf(esc_html__('%s requires SimpleXML or XMLReader extension of PHP to parse WXR file. Make sure to install one of them before importing sample data!', 'ciao'), $this->theme->name); ?>
                    </span>
                </span>
            </td>
            ...
            <td class="field-status">
				...
				<tr>
					<td><?php esc_html_e('PHP Version', 'ciao') ?></td>
					<td>
            <span class="field-tooltip">
                <span class="dashicons dashicons-info"></span>
                <span class="tip-content">
                    <?php printf( esc_html__('%s requires PHP version 5.6 at least.', 'ciao'), $this->theme->name ); ?>
                    <?php esc_html_e('You should use latest version for better perfomance and security.', 'ciao'); ?>
                </span>
            </span>
					</td>
				...
				<tr>
					<td><?php esc_html_e('PHP Memory Limit', 'ciao') ?></td>
					<td>
                        <span class="field-tooltip">
                            <span class="dashicons dashicons-info"></span>
                            <span class="tip-content">
                                <?php esc_html_e('Importing demo data could use large amount of memory.', 'ciao'); ?>
								<?php printf( esc_html__('At peak, %s may require at least 256M of memory.', 'ciao'), $this->theme->name ); ?>
                            </span>
                        </span>
					</td>
				...
				<tr>
					<td><?php esc_html_e('WordPress Version', 'ciao') ?></td>
					<td>
                        <span class="field-tooltip">
                            <span class="dashicons dashicons-info"></span>
                            <span class="tip-content">
                                <?php printf( esc_html__('%s requires WordPress version 4.6 at least.', 'ciao'), $this->theme->name ); ?>
                                <?php esc_html_e('You should use latest version for better perfomance and security.', 'ciao'); ?>
                            </span>
                        </span>
					</td>
            ...
            <td class="field-label"><?php
                esc_html_e('Max Execution Time', 'ciao');
            ?></td>
            <td>
                <span class="field-tooltip" href="#">
                    <span class="dashicons dashicons-info"></span>
                    <span class="tip-content">
                        <?php printf(esc_html__('%s requires at least 30s of PHP execution time.', 'ciao'), $this->theme->name); ?>
                    </span>
                </span>
            </td>
        ...
        <tr>
            ...
            ?>
            <td class="field-label"><?php
                esc_html_e('Max Upload File Size', 'ciao');
            ?></td>
            <td>
                <span class="field-tooltip" href="#">
                    <span class="dashicons dashicons-info"></span>
                    <span class="tip-content">
                        <?php printf(esc_html__('%s requires at least 2M of total upload file size.', 'ciao'), $this->theme->name); ?>
                    </span>
                </span>
            </td>
            ...
            <td class="field-status">
               ...
               <tr>
                    ...
                    ?>
                    <td class="field-label"><?php
                        esc_html_e('Server Max Post Size', 'ciao');
                    ?></td>
                    <td>
                        <span class="field-tooltip" href="#">
                            <span class="dashicons dashicons-info"></span>
                            <span class="tip-content"><?php
                                printf( esc_html__('%s requires at least 8M of total file size.', 'ciao'), $this->theme->name );
                            ?></span>
                        </span>
                    </td>
		...
		<?php if (!$this->system_status) : ?>
			<p class="danger-text"><strong><?php esc_html_e('Whoops, your system does not meet the requirements. You should contact your hosting or VPS provider to fix it before installing this theme.', 'ciao') ?></strong></p>
		<?php endif; ?>
		<p class="envato-setup-actions step">
			<?php
				if ($this->system_status) :
					?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Continue', 'ciao' ); ?></a><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'ciao' ); ?></a><?php
				else :
					?><a href="<?php echo esc_url( admin_url() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Abort', 'ciao' ); ?></a><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Proceed anyway', 'ciao' ); ?></a><?php
				endif;
			?>
		</p>
		<?php
	...
	protected function child_theme()
	{
		...
		<p id="envato-setup-child-theme-text" class="<?php echo esc_attr($class_attr) ?>"><?php
			if ($is_child_theme) {
				printf( esc_html__('%sGreat! A child theme named as %s has already been created and activated successfully%s.', 'ciao'), '<strong>', $child_theme_name, '</strong>' );
			} else {
				printf( esc_html__('If you are going to make changes, it&#8217;s better to use a %schild theme%s rather than modifying parent theme directly. By using a child theme, parent theme can be updated without overwriting your modifications. To generate a child theme, just click on the "Create child theme" button. After generated, feel free to make any change with that child theme which stored in the newly created child theme directory!', 'ciao'), '<a href="https://codex.wordpress.org/Child_Themes" target="_blank">', '</a>' );
			}
		?></p>
		<form action="" method="post">
		 	<div class="child-theme-input">
		 		<label class="screen-reader-text" for="child_theme_name"><strong><?php esc_html_e('Child Theme Name', 'ciao') ?></strong></label>
			...
			<p class="envato-setup-actions step"><?php
				if (!$is_child_theme) :
			    	?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_child"><?php esc_html_e( 'Create child theme', 'ciao' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'ciao' ); ?></a><?php
					wp_nonce_field('envato-setup');
				else :
					?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large"><?php esc_html_e( 'Continue', 'ciao' ); ?></a><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'ciao' ); ?></a>
	...
	protected function default_plugins()
	{
		...
		<form action="" method="post"><?php
			...
			if ( $count ) :
				?><p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated', 'ciao' ); ?>:</p>
				...
				?><p class="lead success"><strong><?php esc_html_e( 'Great! All plugins has already been installed and up to date.', 'ciao' ) ?></strong></p><?php
			endif;
			?><p><?php printf( esc_html__( 'Note that you can add and remove plugins from within %sWordPress admin dashboard%s whenever you want.', 'ciao' ), '<a href="'.admin_url('plugins.php').'">', '</a>' ); ?></p>
			<p class="envato-setup-actions step">
				<?php
					if ($count) :
						?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php esc_html_e( 'Install plugins', 'ciao' ); ?></a><?php
					else :
						?><a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large"><?php esc_html_e( 'Continue', 'ciao' ); ?></a><?php
					endif;
				?>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'ciao' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
	...
	protected function done_setup()
	{
		...
		<form action="" method="post"><?php
			if ( $this->is_possible_upgrade() ) :
				?><p><?php esc_html_e( 'It looks like you already have content imported. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'ciao' ); ?></p><?php
			elseif (true !== $this->tgmpa->is_tgmpa_complete()) :
				?><p><?php echo sprintf(esc_html__( 'Look like required plugins are not installed. Sample data cannot be installed without required plugins! Go to %sinstall plugins%s to install required plugins.', 'ciao' ), '<a href="'.esc_url($this->get_step_link('default_plugins')).'">', '</a>'); ?></p><?php
                return;
            else:
				?><p class="lead success"><strong><?php esc_html_e( 'Cool! Theme and Plugins have been setup successfully!', 'ciao' ) ?></strong></p><p><?php printf(esc_html__( 'It&#8217;s time to import demo data for your new website to make it look even better. %sNOTE THAT BY INSTALLING DEMO DATA, YOUR CURRENT DATA MAY BE OVERRIDDEN OR DUPLICATED%s. We recommend to import demo data on fresh sites or use %sWP Reset plugin%s to clean up your database before importing demo data.', 'ciao' ), '<strong style="color:#ff5042">', '</strong>', '<a href="https://wordpress.org/plugins/wp-reset/" target="_blank">', '</a>'); ?></p><?php
			endif;
			?>
			<br>
			<p class="envato-setup-actions step">
				<a href="<?php menu_page_url('zoo-theme-setup-demo'); ?>" class="button-primary button button-large"><?php esc_html_e( 'Setup Demo Data Now', 'ciao' ); ?></a>
				<a href="<?php echo esc_url( admin_url() ); ?>" class="button button-large"><?php esc_html_e( 'Not right now', 'ciao' ); ?></a>
			</p>
		</form><?php
	}
	...
	protected function demo()
	{
        ...
        ?><form action="" method="post">
			<h2><?php esc_html_e('Select site demo', 'ciao') ?></h2>
			...
			<p><em><?php esc_html_e('If you are unsure to choose which style, just leave to it default. This works best on fresh new sites.', 'ciao') ?></em></p>
            <p class="envato-setup-actions step">
                <input type="submit" class="button-primary button button-large" value="<?php esc_attr_e( 'Continue', 'ciao' ); ?>" name="save_step" />
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large"><?php esc_html_e( 'Skip this step', 'ciao' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
            </p>
        </form><?php
	...
	{
		?><h1 id="zoo-heading-finish"><?php esc_html_e( 'Congratulations! Your website is ready!', 'ciao' ); ?></h1>
		<p><?php esc_html_e('Theme has been installed successfully. Go back to your WordPress dashboard to customize the default content and appearance to suit your needs.', 'ciao') ?></p>
		<div class="envato-setup-next-steps">
			<div class="envato-setup-next-steps-first">
				<h2 ><?php esc_html_e( 'Next Steps', 'ciao' ); ?></h2>
				<ul>
					<li class="setup-product">
						<a class="button button-primary button-large" href="https://themeforest.net/downloads" target="_blank"><?php esc_html_e( 'Rate this theme', 'ciao' ); ?></a>
					</li>
					<li class="setup-product"><a class="button button-large" href="<?php echo esc_url( home_url('/') ); ?>"><?php esc_html_e( 'View your website', 'ciao' ); ?></a></li>
					<li class="setup-product"><a class="button button-large" href="<?php echo esc_url(admin_url('customize.php')); ?>"><?php esc_html_e( 'Customize your website', 'ciao' ); ?></a></li>
				</ul>
			</div>
			<div class="envato-setup-next-steps-last">
				<h2><?php esc_html_e( 'More Steps', 'ciao' ); ?></h2>
				<ul>
					<li class="howto">
						<a href="https://codex.wordpress.org/New_To_WordPress_-_Where_to_Start" target="_blank"><?php esc_html_e( 'Learn WordPress', 'ciao' ); ?></a>
					</li>
					<li class="documentation">
						<a href="http://doc.zootemplate.com/<?php echo get_option('template') ?>" target="_blank"><?php esc_html_e( 'Read Documentation', 'ciao' ); ?></a>
					</li>
					<li class="documentation">
						<a href="<?php echo esc_url('https://themeforest.net/page/item_support_policy') ?>" target="_blank"><?php esc_html_e( 'Item Support Policy', 'ciao' ); ?></a>
					</li>
					<li class="support">
						<a href="<?php echo esc_url('http://member.zootemplate.com/helpdesk') ?>" target="_blank"><?php esc_html_e( 'Get Help and Support', 'ciao' ); ?></a>
					</li>
				</ul>
			</div>
		</div><?php
	...
	protected function get_base_content()
	{
		...
		if ( file_exists($base_dir . 'content.xml') ) {
            $content['content'] = array(
				'title'            => esc_html__('Content', 'ciao'),
				'description'      => esc_html__('Sample content such as posts, pages, menus...', 'ciao'),
				'pending'          => esc_html__('Pending', 'ciao'),
				'installing'       => esc_html__('Installing', 'ciao'),
				'success'          => esc_html__('Success', 'ciao'),
		...
		if ( file_exists($base_dir . 'slider.zip') ) {
			$content['sliders'] = array(
				'title'            => esc_html__('Sliders', 'ciao'),
				'description'      => esc_html__('Sample sliders.', 'ciao'),
				'pending'          => esc_html__('Pending', 'ciao'),
				'installing'       => esc_html__('Installing', 'ciao'),
				'success'          => esc_html__('Success', 'ciao'),
		...
		if ( file_exists($base_dir . 'customizer.dat') ) {
			$content['options'] = array(
				'title'            => esc_html__('Thememods', 'ciao'),
				'description'      => esc_html__('Sample theme customize data.', 'ciao'),
				'pending'          => esc_html__('Pending', 'ciao'),
				'installing'       => esc_html__('Installing', 'ciao'),
				'success'          => esc_html__('Success', 'ciao'),
        ...
        if ( file_exists($base_dir . 'widgets.wie') ) {
            $content['widgets'] = array(
                'title'            => esc_html__('Widgets', 'ciao'),
                'description'      => esc_html__('Sample widgets.', 'ciao'),
                'pending'          => esc_html__('Pending', 'ciao'),
                'installing'       => esc_html__('Installing', 'ciao'),
                'success'          => esc_html__('Success', 'ciao'),
		...
		if ( file_exists($base_dir . 'settings') ) {
			$content['settings'] = array(
				'title'            => esc_html__('Settings', 'ciao'),
				'description'      => esc_html__('Sample theme and plugins&#8217; settings.', 'ciao'),
				'pending'          => esc_html__('Pending', 'ciao'),
				'installing'       => esc_html__('Installing', 'ciao'),
				'success'          => esc_html__('Success', 'ciao'),
