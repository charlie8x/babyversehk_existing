...
add_action('widgets_init', function () {
    for ($i = 1; $i <= 6; $i++) {
        register_sidebar([
            'name' => sprintf(esc_html__('Footer Widget %d', 'ciao'), $i),
            'id' => 'footer-widget-' . $i,
            'description' => sprintf(esc_html__('To display widgets on the site footer.', 'ciao'), $i),
    ...
    for ($i = 1; $i <= 5; $i++) {
        register_sidebar([
            'name' => sprintf(esc_html__('Header Widget %d', 'ciao'), $i),
            'id' => 'header-widget-' . $i,
            'description' => sprintf(esc_html__('To display widgets on the site header.', 'ciao'), $i),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget' => '</section>',
            'before_title' => '<h4 class="widget-title">',
            'after_title' => '</h4>',
...
if (!function_exists('zoo_ajax_get_wishlist_items')) {
    function zoo_ajax_get_wishlist_items()
    {
        if (empty($_POST['wishlistItems'])) {
            wp_send_json(['html' => '<p>' . esc_html_e('Wishlist is empty.', 'ciao') . '</p>']);
        }
                                ...
                                <div class="content-mobile">
                                    ...
                                    <div class="product-meta">
                                        ...
                                        if ($stock_status == 'instock') {
                                            esc_html_e('In stock', 'ciao');
                                        } else if ($stock_status == 'onbackorder') {
                                            esc_html_e('Back Order', 'ciao');
                                        } else {
                                            esc_html_e('Out of stock', 'ciao');
                                        }
                                        ?>
                                        </span>
                                        <?php
...
if (!function_exists('zoo_ajax_get_compare_items')) {
    function zoo_ajax_get_compare_items()
    {
        if (empty($_POST['compareItems'])) {
            wp_send_json(['html' => '<p>' . esc_html_e('No products to compare.', 'ciao') . '</p>']);
        }
        ...
        if ($query->have_posts()) : ?>
            <h2 class="products-compare-panel-title zoo-popup-panel-title">
                <?php echo esc_html__('Compare Products', 'ciao') ?>
            </h2>
            <div class="zoo-wrap-popup-content">
                <table class="products-compare-table">
                    ...
                    <tr>
                        <th><?php esc_html_e('Rating', 'ciao') ?></th>
                        <?php while ($query->have_posts()) : $query->the_post(); ?>
                    ...
                    <tr>
                        <th><?php esc_html_e('Price', 'ciao') ?></th>
                        <?php while ($query->have_posts()) : $query->the_post(); ?>
                    ...
                    <?php if (!zoo_enable_catalog_mod()) { ?>
                        <tr>
                            <th><?php esc_html_e('Add to cart', 'ciao') ?></th>
                            <?php while ($query->have_posts()) : $query->the_post(); ?>
                    ...
                    <?php } ?>
                    <tr>
                        <th><?php esc_html_e('Description', 'ciao') ?></th>
                        <?php while ($query->have_posts()) : $query->the_post(); ?>
                    ...
                    <tr class="attribute-head-group-row">
                        ...
                        ?>
                        <th><?php esc_html_e('Attributes', 'ciao') ?></th>
