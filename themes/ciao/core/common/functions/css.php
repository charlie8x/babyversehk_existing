...
function zoo_import_google_fonts(array $list_fonts, $load = true)
{
    ...
    $subsets[] = 'latin';
    $subsets[] = 'latin-ext';
    $subset    = _x('no-subset', 'Add new subset (greek, cyrillic, devanagari, vietnamese)', 'ciao');
...
function zoo_get_font_icons($prefix = '')
{
    ...
    if ('' === $prefix) {
        ...
        throw new Exception(sprintf(esc_html__('Font with the prefix "%s" not found!', 'ciao'), $prefix));
    }
}
