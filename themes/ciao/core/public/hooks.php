    ...
    if (class_exists('WooCommerce', false)) {
    	if(get_theme_mod('zoo_enable_wishlist','1')){
		    ...
		    wp_localize_script('zoo-wishlist', 'zooWishlistCDATA', [
			    'addToWishlist' => get_theme_mod('zoo_text_add_to_wishlist', esc_html__('Add to Wishlist', 'ciao')),
			    'addToWishlistIcon' => $add_to_wishlist_icon,
			    'browseWishlist' => get_theme_mod('zoo_text_browse_to_wishlist', esc_html__('Browse Wishlist', 'ciao')),
			    'browseWishlistIcon' => $browse_to_wishlist_icon,
			    'addToWishlistErr' => esc_html__('Failed to add the item to Wishlist.', 'ciao'),
			    'wishlistIsEmpty' => esc_html__('Wishlist is empty.', 'ciao')
		    ]);
	    }
	    if(get_theme_mod('zoo_enable_compare','1')) {
		    ...
		    wp_localize_script( 'zoo-products-compare', 'zooProductsCompareCDATA', [
			    'addToCompare'      => get_theme_mod( 'zoo_text_add_to_compare', esc_html__( 'Add to Compare', 'ciao' ) ),
			    'addToCompareIcon'  => $add_to_compare_icon,
			    'browseCompare'     => get_theme_mod( 'zoo_text_browse_to_compare', esc_html__( 'Browse Compare', 'ciao' ) ),
			    'browseCompareIcon' => $browse_to_compare_icon,
			    'addToCompareErr'   => esc_html__( 'Failed to add the item to compare list.', 'ciao' ),
			    'compareIsEmpty'    => esc_html__( 'No products to compare.', 'ciao' )
		    ] );
	    }
    }
