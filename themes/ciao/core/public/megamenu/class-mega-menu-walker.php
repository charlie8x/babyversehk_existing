...
class Zoo_Mega_Menu_Walker extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = [], $id = 0)
    {
        ...
        if ($item_hot || $item_new || $item_trending) {
            if ($item_hot) {
                $badge[] = '<span class="hot-badge items-badge">' . esc_html__('Hot', 'ciao') . '</span>';
            }
            if ($item_new) {
                $badge[] = '<span class="new-badge items-badge">' . esc_html__('New', 'ciao') . '</span>';
            }
            if ($item_trending) {
                $badge[] = '<span class="trending-badge items-badge">' . esc_html__('Trending', 'ciao') . '</span>';
            }
        }
        if ($depth == 1 && intval($this->isEnabled)) {
