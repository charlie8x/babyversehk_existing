(function ($, w) {
    ...
    /*Check User login*/
    $('.zoo-account-block .woocommerce-form-login').on('submit', function (e) {
        e.preventDefault();
        var user = {};
        var $form = $(this);
        ...
        $form.closest('.wrap-login-form').addClass('zoo-loading');
        $.ajax({
            url: ajaxurl,
            type: "POST",
            data: {
                action: "zoo_user_login",
                loginParam: JSON.stringify(user)
            }
        }).done(function (result) {
            if (!!result) {
                if ($form.find('.zoo-notice')[0]) {
                    $form.find('.zoo-notice').html(result);
                } else {
                    $form.prepend('<p class="zoo-notice">' + result + '</p>');
                }
                $form.closest('.wrap-login-form').removeClass('zoo-loading');
            } else {
