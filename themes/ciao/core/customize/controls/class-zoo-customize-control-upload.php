<?php
/**
 * Zoo_Customize_Control_Upload
 *
 * @package  Zoo_Theme\Core\Customize\Classes\Controls
 *
 */
final class Zoo_Customize_Control_Upload extends Zoo_Customize_Control_Media
{
    /**
     * Print template
     */
    ...
    {
        ?>
        <script type="text/html" id="tmpl-zoo-customize-control-upload">
        <#
        var required = '';
        ...
        <div class="zoo-customize-control zoo-customize-control-{{ data.type }} {{ data.class }} zoo-customize-control-name-{{ data.original_name }}" data-required="{{ required }}" data-field-name="{{ data.name }}">
            ...
            <div class="zoo-customize-control-settings-inner zoo-media-type-{{ data.type }}">
                <div class="zoo-media">
                    ...
                    <div class="zoo-image-preview <# if ( url ) { #> zoo-has-file <# } #>" data-no-file-text="<?php esc_attr_e( "No file selected", 'ciao' ); ?>">
                        <#
                    ...
                    <button type="button" class="button zoo-add <# if ( url ) { #> zoo-hide <# } #>"><?php esc_html_e( 'Add', 'ciao' ); ?></button>
                    <button type="button" class="button zoo-change <# if ( ! url ) { #> zoo-hide <# } #>"><?php esc_html_e( 'Change', 'ciao' ); ?></button>
                    <button type="button" class="button zoo-remove <# if ( ! url ) { #> zoo-hide <# } #>"><?php esc_html_e( 'Remove', 'ciao' ); ?></button>
                </div>
            </div>
        </div>
        </script>
