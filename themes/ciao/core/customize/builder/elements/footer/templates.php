...
final class Zoo_Customize_Builder_Element_Footer_Templates extends Zoo_Customize_Builder_Element
{
    ...
    public function get_customize_configs(WP_Customize_Manager $wp_customize = null)
    {
        ...
        $option_name        = get_option('template', 'ciao').'_footer_saved_templates';
        $active_tpl         = get_theme_mod('active_footer_template');
        $saved_templates    = get_option($option_name, []);
        $prebuilt_templates = get_option(get_option('template', 'ciao').'_footer_prebuilt_templates', []);
        ...
        if ($tpl_count > 0) {
            foreach ($saved_templates as $key => $tpl) {
                ...
                $html .= '<li class="saved_template li-boxed'.$active_class.'" data-tpl-id="'.esc_attr($key).'" data-builder-id="footer" data-builder-panel="footer_builder_panel">'.esc_html($tpl['name']).' <a href="#" class="delete-tpl" title="'.esc_attr__('Delete Template', 'ciao').'"><i class="dashicons dashicons-trash"></i></a><a href="?export-tpl='.esc_attr($key).'&export-tpl-builder=footer" class="export-tpl" title="'.esc_attr__('Export Template', 'ciao').'"><i class="dashicons dashicons-upload"></i></a><a href="#" class="import-tpl" title="'.esc_attr__('Install Template', 'ciao').'"><i class="dashicons dashicons-download"></i></a></li>';
            }
        }
        ...
        if (!empty($theme_options['enable_dev_mode'])) {
            $fields[] = [
                ...
                'description'    => '<div class="save-template-form"><p class="description customize-control-description">'.esc_html__('Import a footer template and use it later.', 'ciao').'</p><div class="zoo-flexbox"><input type="file" data-builder-id="footer" data-builder-panel="footer_builder_panel"  class="zoo-customize-import-template-file" name="zoo-customize-import-file" accept="*.json"><button type="button" class="button button-primary zoo-customize-import-template-button">'.esc_html__('Import', 'ciao').'</button></div></div>',
            ];
        }
        ...
        $fields[] = [
            ...
            'description'    => '<div class="save-template-form"><p class="description customize-control-description">'.esc_html__('Save current footer layout and use it later.', 'ciao').'</p><div class="zoo-flexbox"><input type="text" data-builder-id="footer" data-builder-panel="footer_builder_panel" class="template-input-name change-by-js"><button class="button button-primary save-builder-template" type="button">'.esc_html__('Save', 'ciao').'</button></div></div>'.$html
        ];
