...
abstract class Zoo_Customize_Builder_Element
{
    ...
    function __get($name)
    {
        if (!isset($this->$name) || !is_string($name)) {
            throw new InvalidArgumentException(__('Invalid builder element property!', 'ciao'));
        }
    ...
    protected function get_layout_configs($merge_selector)
    {
        return [
            [
                ...
                'title' => esc_html__('Element Border', 'ciao'),
                'priority' => 900,
            ],
            [
            ...
            [
                ...
                'title' => esc_html__('Merging', 'ciao'),
                'description' => esc_html__('If you choose to merge this item, the alignment setting will inherit from the item you are merging.', 'ciao'),
                'choices' => [
                    0 => esc_html__('No', 'ciao'),
                    'prev' => esc_html__('Merge with left item', 'ciao'),
                    'next' => esc_html__('Merge with right item', 'ciao'),
                ],
                'priority' => 909,
            ]
        ];
