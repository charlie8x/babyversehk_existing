...
class Zoo_Customize_Builder
{
    ...
    public function add_element($builder_id, Zoo_Customize_Builder_Element $class)
    {
        if (!$builder_id || !isset($this->registered_builders[$builder_id])) {
            throw new Exception(__('Invalid builder panel!', 'ciao'));
        }
    ...
    public function _ajax_save_template()
    {
        if (!current_user_can('edit_theme_options')) {
            wp_send_json_error(__('Access denied', 'ciao'));
        }

        $panel_id        = sanitize_text_field($_POST['panel']);
        $builder_id      = sanitize_text_field($_POST['builder']);
        $option_name     = get_option('template', 'ciao').'_'.$builder_id.'_saved_templates';
        $saved_templates = get_option($option_name, []);

        if (!isset($this->registered_builders[$builder_id])) {
            wp_send_json_error(__('Invalid builder ID.', 'ciao'));
        }
        ...
        if (isset($_POST['remove'])) {
            ...
            if (empty($_FILES['zooCustomizeTemplateFile']['tmp_name']) || !empty($_FILES['zooCustomizeTemplateFile']['error'])) {
                wp_send_json_error(esc_html__('Invalid import template file.', 'ciao'));
            }
            ...
            if (!empty($data['thememods']) && !empty($data['name'])) {
                ...
                $data['name'] = '['.esc_html__('Imported', 'ciao').'] '.$data['name'];
                $saved_templates[$tpl_key] = $data;
                update_option($option_name, $saved_templates);
                $html = '<li class="saved_template li-boxed" data-tpl-id="'.esc_attr($tpl_key).'" data-builder-id="'.esc_attr($builder_id).'" data-builder-panel="'.esc_attr($panel_id).'">' . esc_html($data['name']) . ' <a href="#" class="delete-tpl"><i class="dashicons dashicons-trash"></i></a><a href="?export-tpl='.esc_attr($tpl_key).'&export-tpl-builder='.esc_attr($builder_id).'" class="export-tpl" title="'.esc_attr__('Export Template', 'ciao').'"><i class="dashicons dashicons-upload"></i></a><a href="#" class="import-tpl"><i class="dashicons dashicons-download"></i></a></li>';
            ...
            $html = '<li class="saved_template li-boxed active-builder-template" data-tpl-id="'.esc_attr($tpl_key).'" data-builder-id="'.esc_attr($builder_id).'" data-builder-panel="'.esc_attr($panel_id).'">'.esc_html($tpl_name) . ' <a href="#" class="delete-tpl"><i class="dashicons dashicons-trash"></i></a><a href="?export-tpl='.esc_attr($tpl_key).'&export-tpl-builder='.esc_attr($builder_id).'" class="export-tpl" title="'.esc_attr__('Export Template', 'ciao').'"><i class="dashicons dashicons-upload"></i></a><a href="#" class="import-tpl"><i class="dashicons dashicons-download"></i></a></li>';
    ...
    public function _ajax_update_template()
    {
        ...
        if (!$active_template || $active_template !== $template_id || !isset($saved_templates[$active_template])) {
            wp_send_json_error(__('Invalid active template ID.', 'ciao'));
        }
        ...
        if ($updated) {
            wp_send_json_success(esc_html__('Active template updated succesfully!', 'ciao'));
        } else {
            wp_send_json_error(__('Failed to update the active template.', 'ciao'));
        }
    }
    ...
    public function _ajax_install_template()
    {
        ...
        $mod_key     = is_child_theme() ? 'theme_mods_'.ZOO_CHILD_THEME_SLUG : 'theme_mods_'.get_option('template', 'ciao');
        ...
        if ($panel_id !== $tpl_data['panel_id']) {
            wp_send_json_error(__('Invalid panel ID.', 'ciao'));
        }
        ...
        if (!add_option('sidebars_widgets', $_widgets)) {
            wp_send_json_error(__('Failed to update widgets location.', 'ciao'));
        }

        delete_option($mod_key);

        if (!add_option($mod_key, $mods)) {
            wp_send_json_error(__('Failed to update theme mods.', 'ciao'));
        }
    ...
    function _export_template()
    {
        ...
        $templates = get_option(get_option('template', 'ciao').'_'.$builder_id.'_saved_templates', []);
        ...
        <div class="zoo-customize-builder-panel">
            <div class="zoo-customize-builder-inner">
                <div class="zoo-customize-builder-header">
                    <div class="zoo-customize-builder-label"> <# if (data.id === 'header') { #><?php esc_html_e('Header Builder','ciao')?>
                        <# }else{ #>
                        <?php esc_html_e('Footer Builder','ciao')?>
                        <# } #></div>
                    <div class="zoo-customize-builder-devices-switcher"></div>
                    <div class="zoo-customize-builder-actions">
                        <# if (data.id === 'header') { #>
                        <a data-id="header_sticky" class="zoo-customize-focus-button button button-secondary" href="javascript:void(0)"><?php esc_html_e('Header Sticky', 'ciao'); ?></a>
                        <# } #>
                        <a data-id="{{ data.id }}_builder_style" class="zoo-customize-focus-button button button-secondary" href="javascript:void(0)"><?php esc_html_e('Style', 'ciao'); ?></a>
                        <a data-id="{{ data.id }}_templates" class="zoo-customize-focus-button button button-secondary" href="javascript:void(0)"><?php esc_html_e('Templates', 'ciao'); ?></a>
                        <a class="button button-secondary zoo-customize-builder-panel-minimize" href="javascript:void(0)">
                            <span class="zoo-customize-builder-minimize-text"><?php esc_html_e('Minimize', 'ciao'); ?></span>
