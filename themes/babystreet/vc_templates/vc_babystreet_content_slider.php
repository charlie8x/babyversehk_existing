 ...
 * @var $this WPBakeryShortCode_Babystreet_Content_Slider|WPBakeryShortCode_VC_Tta_Tabs|WPBakeryShortCode_VC_Tta_Tour|WPBakeryShortCode_VC_Tta_Pageable
 *
...
// Full Height option
if($full_height === 'yes') {
	$css_classes[] = 'babystreet-fullheight-content-slider';
}
...
$unique_id = uniqid('babystreet_content_slider');

$output_escaped = '<div id="' . esc_attr($unique_id) . '" class="babystreet_content_slider' . ($custom_css_class ? ' ' . esc_attr($custom_css_class) : '') . (empty($css_classes) ? '' : ' ' . esc_attr(implode(' ', $css_classes))) . '">';
...
$inline_js = '(function ($) {
		"use strict";
		var is_babystreet_video_background = jQuery("#' . esc_js($unique_id) . '").find("div.babystreet_bckgr_player").length;
		...
		$(window).on("load", function () {
			...
			setTimeout(function(){ jQuery("#' . esc_js($unique_id) . ' > .vc_tta-panels").owlCarousel(babystreet_owl_args) }, 10)
		});
	})(window.jQuery);';
wp_add_inline_script('owl-carousel', $inline_js);

// This variable has been safely escaped in the following file: babystreet/vc_templates/vc_babystreet_content_slider.php Line: 40 - 47
