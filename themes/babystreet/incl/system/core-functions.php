	...
	function babystreet_register_required_plugins() {
		...
		$plugins = array(
				...
				array(
						...
						'required' => false, // If false, the plugin is only 'recommended' instead of required
						'version' => '4.8.0', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
				),
				array(
						'name' => esc_html__('YITH WooCommerce Wishlist', 'babystreet'), // The plugin name
						'slug' => 'yith-woocommerce-wishlist', // The plugin slug (typically the folder name)
						'required' => false, // If false, the plugin is only 'recommended' instead of required
						'version' => '3.0.17', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
				),
				array(
...
if (!function_exists('babystreet_enqueue_scripts_and_styles')) {
	...
	function babystreet_enqueue_scripts_and_styles() {
		...
		/* Include js configs */
        $libs_config_dependant_scripts = array('jquery', 'wp-util', 'flexslider', 'owl-carousel', 'cloud-zoom', 'countdown', 'magnific', 'appear', 'typed', 'nice-select', 'is-in-viewport');
		if ($to_include_backgr_video) {
			$libs_config_dependant_scripts[] = 'ytplayer';
		}
		if ($to_include_supersized) {
			$libs_config_dependant_scripts[] = 'supersized-min';
		}

		wp_enqueue_script('babystreet-libs-config', get_template_directory_uri() . "/js/babystreet-libs-config.min.js", $libs_config_dependant_scripts, false, true);
...
if (!function_exists('babystreet_check_for_sidebar')) {
	...
	function babystreet_check_for_sidebar() {
		...
		if (BABYSTREET_IS_WOOCOMMERCE && function_exists('is_woocommerce') && is_woocommerce() && isset($woocommerce_sidebar)) {
