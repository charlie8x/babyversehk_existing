...
if (!function_exists('babystreet_post_nav')) {
	...
	function babystreet_post_nav($same_category = false, $taxonomy = 'category') {
		...
		if (version_compare($wp_version, '3.8', '>=')) {
			$entries['prev'] = array(
				'key_label' => esc_html__( 'prev', 'babystreet' ),
				'near_post' => get_previous_post( $same_category, $excluded_terms, $taxonomy )
			);
			$entries['next'] = array(
				'key_label' => esc_html__( 'next', 'babystreet' ),
		...
		foreach ($entries as $key => $entry) {
			...
			$the_title = babystreet_generate_excerpt(get_the_title($entry['near_post']->ID), 75, " ", " ", true, '', true);
...
if ( ! function_exists( 'babystreet_comments_are_valid_for_post' ) ) {
	/**
	 ...
	 */
	function babystreet_comments_valid_for_post( $post_id ) {
...
add_filter( 'wcml_multi_currency_ajax_actions', 'babystreet_add_action_to_multi_currency_ajax', 10, 1 );
if (!function_exists('babystreet_add_action_to_multi_currency_ajax')) {
	/**
	 ...
	 * @return array
	 */
	function babystreet_add_action_to_multi_currency_ajax( $ajax_actions ) {

		$ajax_actions[] = 'wptf_fragment_refresh'; // Add a AJAX action to the array
		$ajax_actions[] = 'wptf_ajax_add_to_cart';

		// Babystreet actions below
		$ajax_actions[] = 'babystreet_quickview';
		$ajax_actions[] = 'babystreet_wc_add_cart';

		return $ajax_actions;
	}
}
add_filter( 'yith_wcwl_localize_script', 'babystreet_add_wishlist_settings', 99 );
if ( ! function_exists( 'babystreet_add_wishlist_settings' ) ) {
	/**
	 ...
	 * @return array
	 */
	function babystreet_add_wishlist_settings( $wishlist_settings_array ) {
